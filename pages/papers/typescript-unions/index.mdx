...
This process is call Discrimination. It's the other half of the solution. With simple entities such as the one below it's pretty straightforward. However, discriminating objects can be tough.
...
But, sometimes it's not possible to rely upon one specific field. Maybe it's not even up to us to decide the object shapes - working with a third-party API. In such cases, we can infer the type by running a morphism check. Meaning, looking for discrimination in their shape.
...
Finally, when everything failed. We can use a to-be-defined function. If the objects don't have an identifier field. If they are morphally equal. We can check their inner differences with a function. And let this TBD function to imperatively define the type.

```ts
type VaporCoin = { key: string }
type NeonCoin = { key: string }

const isVapor = (tbd: any): tbd is VaporCoin => {
...
4. If that's not possible, use morphism checks.
