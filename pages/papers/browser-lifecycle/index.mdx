...
Any website follows a series of steps involving at least one client, one server and a myriad of infrastructure proxies. A word of advice - in order to keep it focused - in this post I'm not gonna discuss the basics of the Internet, HTTP requests, etc. [In short](https://developer.mozilla.org/en-US/docs/Web/Performance/How_browsers_work):

1. The client (i.e. a browser) **requests access** to a website.
...
/* Does a Fetch o Fetches */
7. Parses and **evaluates JavaScript**.
8. The **website is ready** to be used.
9. Maybe, **keeps loading** more resources.
...
7. The browser has fully loaded HTML, and the **DOM tree is built**. External resources like pictures `<img>` and stylesheets **MAY** not yet have loaded.
...
11. Other **subsequent deferred requests** might finish and be executed later on.

This is just the theory. How can we use these events in our benefit? Well, all browsers expose a series of states that we can hook of. These are the most useful:
/* hook of? Quizás hook to quedaría mejor, qué quieres decir? */
### Vanilla
...
_**✨ Bonus round:** It's not the same loading the `script` at the `head`. Or at the end of the `body`, or with attributes like `defer` or `async`. Checkout this Codepen._
/* There is no codepen link bro!*/

### DOM Content Loaded

**When:** At step 7. It's the main browser event since it marks the readiness for any kind of JavaScript to start working.
...
_**✨ Bonus round**: Does your employer force you to drink sparkling water and support IE8? Here's the alternative magic:_
