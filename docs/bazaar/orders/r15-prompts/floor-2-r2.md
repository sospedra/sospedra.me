# Codex subagent order — Bazaar 3 Floor 2 master (round 2: rendering normalization)

You are an image-generation subagent. Execute exactly this task and nothing else.

The attached images are, in order:

- Image 1: EDIT TARGET — the round-1 Floor 2 master.
...
3. Copy the final PNG unmodified to exactly:
   /Users/sospedra/labs/sospedra.me/tmp/bazaar3/master-run-20260728/candidates/floor-2-r2.png
...
poses (floating three-eye three-arm robot; Ed cross-legged on the rug with
visor; deadpan clerk cheek on hand), sign shapes and positions, prop placement
and light-source positions EXACTLY as they are in Image 1. This pass changes
rendering and one listed correction, nothing else.
...
speckle, micro-shading and concrete noise into those flat fields. Strengthen
continuous near-black outlines around every stall, character, structure and
prop. Keep tiny hard highlights only on focal edges (faces, hands, eye stalks,
claws, screens, sign letters, inventory). Quiet large surfaces: concrete,
steel, wood and rug read as flat bands, not textured planes.
...
1. Raise the legibility of the left spiral stair by one value step of contrast
   between the treads/rail and the dark recess, so the stair reads at small
   preview size. Same position, same size, same design.
2. Nothing else moves, appears or disappears.

Lighting stays causal and flat: Manual's amber work lamp, Console's teal/cyan
screens with the bay darker than its neighbors, Talks' warm pendant and the
SMPTE-bar CRT keep their positions; each source keeps a tiny bright core, one
hard receiver band and one weaker spill band as flat palette steps; compact
contact shadows trend down-right; the lobby stays dark. No glow, bloom, halo,
gradient or noise anywhere.

Text stays exactly: "manual", "console", "VIDEO CLUB", the pink Up arrow
blade, the cyan Down arrow blade. Nothing else readable; terminal text and
posters stay unreadable marks.
