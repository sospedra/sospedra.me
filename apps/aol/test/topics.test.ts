...
test('roomTopic is sha256(appId || roomId || topicSecret)', () => {
  const topic = roomTopic('aol/v0', 'room-1', 's3cret')
...
test('roomTopic changes with every component', () => {
  const base = roomTopic('aol/v0', 'room-1', 's3cret')
  assert.notEqual(roomTopic('aol/v1', 'room-1', 's3cret'), base)
  assert.notEqual(roomTopic('aol/v0', 'room-2', 's3cret'), base)
  assert.notEqual(roomTopic('aol/v0', 'room-1', 'other'), base)
})

test('inviteTopic is sha256(appId || inviteId)', () => {
  const inviteId = Uint8Array.from({ length: 16 }, (_, i) => i + 1)
  const topic = inviteTopic('aol/v0', inviteId)
  const expected = sha256hex(Buffer.from('aol/v0'), Buffer.from(inviteId))
  assert.equal(topic, expected)
})
