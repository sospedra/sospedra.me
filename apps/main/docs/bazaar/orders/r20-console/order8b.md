# Codex order - r20.8b - console master, nest edition (corrective)
...
- Image 3 = ED, the character gospel (a Cowboy Bebop film still).
  ...
  objects.
- Image 4 = YOUR PREVIOUS ATTEMPT. Its materials, character reading
  and inventory quality are APPROVED; its geometry is REJECTED:
  slab angle 62 degrees (must be exactly 50, 68:81), art 900 px
  wide (must be 608), a TV stack behind Ed (must be server rack B),
  a game controller (must be none), goggles on the forehead (must
  be on the eyes with a temple cable). Keep its wins, fix exactly
  these, follow every rule below.
...
unmodified to r20-console/gen8b.png and print GENERATED=<path>.
