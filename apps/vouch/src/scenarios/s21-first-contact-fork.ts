...
function run(): Trace {
  ...
  return {
    ...
    verdict: {
      kind: 'LIMITATION',
      note: `world A and world B both descend from the identical pinned genesis root ${hex(GENESIS_ROOT)}, then diverge at a single transfer amount (100 in world A, 999 in world B); each history is honestly sealed, and a fresh client holding only genesis trust fully verifies EITHER bundle with a clean, unskipped ladder — bob's balance is ${balanceA.balance} in world A and ${balanceB.balance} in world B, and nothing in first-contact verification can tell which one is canonical, or whether more forks exist. ${SPEC_FIRST_CONTACT_LIMIT}`,
    },
  }
}
