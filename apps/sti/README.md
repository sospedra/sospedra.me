...
```sh
pnpm --filter sti test       # node --test
pnpm --filter sti build      # tsc emit to dist/
pnpm --filter sti lint       # biome check
pnpm --filter sti typecheck  # tsc --noEmit
```
