...
| A log that stops publishing is in breach, not idle, and resuming does not clear it: the report holds the WORST gap across all heartbeats, never only the latest | §17.1 | `world/world.ts` `publishHeartbeat`, `world/keyless-verifier.ts` `heartbeatStatus` | `test/keyless.test.ts` |
| A heartbeat names a chain tip the verifier checks against the chain, by hash, so it cannot name a block it never saw | §17.1 | `world/log-records.ts` `heartbeatLeaf` | `test/keyless.test.ts` |
| A witness REFUSES a second root at a size it already cosigned, so splitting the view costs k corrupted witnesses. A stateless cosignature would prove only that k keys saw this head, not that they saw no other | §8.1 | `core/merkle.ts` `countCosignatures` | `test/keyless.test.ts` |
...
| Withholding, padding or reordering the auditor's view is caught by `missingFromView`, `duplicates` and the index-ordered walk. `viewFromLog` is a convenience for a holder of the log, not a guarantee an auditor can invoke | §17.1 | `core/merkle.ts` `serveLeaves`, `world/keyless-verifier.ts` `viewFromLog` | `test/keyless.test.ts` |
