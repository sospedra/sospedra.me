...
import { type MoonPhase, moonPhase, moonTilt } from 'service/moon'
...
let geoRequest: Promise<Geo | null> | null = null

// One lookup per session: home remounts on every revival, the answer is stable
const requestGeo = (): Promise<Geo | null> => {
  geoRequest ??= fetch('/api/geo', { cache: 'no-store' })
    .then((response) =>
      response.ok ? (response.json() as Promise<Geo>) : null,
    )
    .catch(() => {
      geoRequest = null
      return null
