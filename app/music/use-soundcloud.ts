...
export const useSoundCloud = (outputVolume: number) => {
  ...
  const syncCurrentSound = useCallback((widget: SoundCloudWidget) => {
    ...
    widget.getCurrentSound((currentSound) => {
      widget.getCurrentSoundIndex((currentIndex) => {
        ...
        setState((current) => ({
          ...
          currentIndex,
          currentSound,
          duration: currentSound.duration ?? current.duration,
          position:
            current.currentIndex === currentIndex ? current.position : 0,
