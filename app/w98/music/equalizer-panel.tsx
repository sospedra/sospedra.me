...
const levelStyle = (value: number) =>
  cssVars({ '--eq-level': `${((value + 12) / 24) * 100}%` })

const balanceLabel = (balance: number): string => {
  if (balance === 0) return 'Centered'
  if (balance < 0) return `${Math.round(Math.abs(balance) * 100)} percent left`
  return `${Math.round(balance * 100)} percent right`
}
  ...
  return (
    ...
    >
      ...
      <label
        ...
        data-calibration-kind='control'
        data-no-drag
        style={cssVars({ '--balance-level': `${((balance + 1) / 2) * 100}%` })}
      >
        ...
        />
        <output className={css.srOnly}>{balanceLabel(balance)}</output>
      </label>
