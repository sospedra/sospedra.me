...
/* legacy split-scene fallback: papers has no keeper hover frames */
const HOVERLESS: ReadonlySet<Stall2Id> = new Set(['papers'])

const ANIMATED_STALLS: ReadonlySet<Stall2Id> = new Set([
  'uses',
  'papers',
  'games',
  'talks',
...
const BAKED: ReadonlySet<Stall2Id> = new Set(['travel', 'console', 'projects'])
...
function StallInner({ id, bp }: { id: Stall2Id; bp: Bp }) {
  if (ANIMATED_STALLS.has(id)) {
    return <FullStallFrames id={id} />
  }
  if (BAKED.has(id)) {
    const bakedAsset =
      id === 'projects' ? 'stall-projects-baked-v2' : `stall-${id}-baked`
    return (
      <>
        <img src={src(bakedAsset)} alt='' className={styles.fill} />
        <div className={styles.glowWash} />
      </>
    )
  }
    ...
    >
      {mobile && (
        <div className={styles.mobileFloorSplit} aria-hidden>
          ...
          <img
            src={src('stairs-mobile-platform')}
            alt=''
            className={styles.mobileFloorSplitPlatform}
            draggable={false}
            loading='lazy'
          />
        </div>
      ...
      <img
        src={src(mobile ? 'stairs-mobile-column' : `stairs-${id}`)}
        alt=''
...
const DESKTOP_MARKETS: MarketSpec[] = [
  ...
  { stalls: ['projects', 'games', 'travel'], stairs: 3, stairsFirst: true },
]
        ...
        {DESKTOP_MARKETS.map((spec, i) => (
          <Fragment key={spec.stalls[0]}>
            <div
              ...
              style={{
                backgroundImage: `url(/images/bazaar2/assets/slabs/slab-${SLAB_KINDS[i % 3]}.png), url(/images/bazaar2/assets/slabs/slab-${SLAB_KINDS[i % 3]}-bg.png)`,
