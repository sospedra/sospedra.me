...
const SHAPE_TOLERANCE_OVERRIDES = {
  AU: 0.045,
  CA: 0.22,
  CL: 0.075,
  GB: 0.035,
...
const worldProject = ([longitude, latitude]) => [
  round(((longitude + 180) / 360) * 1200),
  round(((90 - latitude) / 180) * 600),
]
...
const polygonPath = (polygon, project, tolerance) =>
  polygon
    .map((ring) => ringPath(simplifyRing(ring, tolerance), project))
    .filter(Boolean)
    .join('')
...
for (const country of shapeCountries) {
  ...
  if (polygons.length === 0) {
    throw new Error(`Natural Earth shape for ${code} has no visible polygons`)
  }
  const project = shapeProjector(polygons)
  const tolerance = SHAPE_TOLERANCE_OVERRIDES[code] ?? 0.04
  const paths = polygons
    .map((polygon) => polygonPath(polygon, project, tolerance))
    .filter(Boolean)
    .map((path) => `<path fill="#e8dfca" fill-rule="evenodd" d="${path}"/>`)
    .join('')
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${SVG_WIDTH} ${SVG_HEIGHT}" preserveAspectRatio="xMidYMid meet">${paths}</svg>\n`
...
const worldPaths = allLandPolygons
  ...
  .map((polygon) => polygonPath(polygon, worldProject, 0.18))
  .filter(Boolean)
  .join('')
const worldSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 600" preserveAspectRatio="xMidYMid meet"><path fill="#a59a78" fill-rule="evenodd" d="${worldPaths}"/></svg>\n`
