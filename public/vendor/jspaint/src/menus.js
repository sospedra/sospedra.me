...
const normalize_menu_label = (label) => label.replace(/&|\.\.\./g, "");

const menu_item_label = (menu_item) => (
	menu_item && typeof menu_item === "object" && "label" in menu_item
		? normalize_menu_label(menu_item.label)
		: ""
);

const select_menu_items = (menu, labels) => labels.map((label) => {
	const normalized_label = normalize_menu_label(label);
	const menu_item = menu.find((candidate) => menu_item_label(candidate) === normalized_label);
	if (!menu_item) {
		throw new Error(`Missing JS Paint menu item: ${normalized_label}`);
	}
	return menu_item;
});

const file_menu_key = localize("&File");
const edit_menu_key = localize("&Edit");

menus[file_menu_key] = select_menu_items(
	menus[file_menu_key],
	[localize("&New"), localize("&Open"), localize("Save &As")],
);

const [undo_item, cut_item, copy_item, paste_item, clear_item, select_all_item] = select_menu_items(
	menus[edit_menu_key],
	[
		localize("&Undo"),
		localize("Cu&t"),
...
for (const hidden_menu of ["&View", "&Image", "&Colors", "&Help", "E&xtras"]) {
	delete menus[localize(hidden_menu)];
}
